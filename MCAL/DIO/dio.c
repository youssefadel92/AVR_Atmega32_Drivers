...
void dio_vidWriteChannelGroup(dio_portId_t port,u8 value,u8 mask)
{
	...
	switch (port)
	{
		case DIO_PORTA:
			DIO_PORTA_REG&=mask;
			DIO_PORTA_REG|=((value<<count)&mask);
			break;
		case DIO_PORTB:
			DIO_PORTB_REG&=mask;
			DIO_PORTB_REG|=((value<<count)&mask);
			break;
		case DIO_PORTC:
			DIO_PORTC_REG&=mask;
			DIO_PORTC_REG|=((value<<count)&mask);
			break;
		case DIO_PORTD:
			DIO_PORTD_REG&=mask;
			DIO_PORTD_REG|=((value<<count)&mask);
			break;
	}
}

/******************************************************************************
* \Syntax          :void dio_vidReadChannelGroup(dio_portId_t port,u8 value,u8 mask)
