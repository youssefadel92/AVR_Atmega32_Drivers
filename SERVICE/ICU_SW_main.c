...
//ISR(INT0_vect)
{
	if (flag == 0) // first rising edge
	{
		EXT0_voidSetSignalch(FALLING);
		flag++;
	}
	else if (flag == 1) // second falling edge
	{
		ton = ((ovf * 256.0) + TCNT0)*1024.0/F_CPU;
		flag++;
		EXT0_voidSetSignalch(RISING);
	}
	else if(flag==2) // third rising edge
	{
		toff = ((ovf * 256.0) + TCNT0)*1024.0/F_CPU;
		EXT0_voidSetSignalch(FALLING);
		flag = 1;
	}
	TCNT0=0;
	ovf = 0; // reset overflow counter
}
